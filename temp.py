...
def perform_ocr(image_file):
    try:
        ...
        raise ValueError("OCR processing failed. Please ensure the image is valid and readable.")

def process_excel_file(file):
    wb = openpyxl.load_workbook(file)
    return "\n".join(" | ".join(map(str, row)) for sheet in wb.sheetnames for row in wb[sheet].iter_rows(values_only=True))
...
def translate_text(text: str, target_lang: str) -> str:
    return text if target_lang == "English" else GoogleTranslator(source="auto", target=target_lang).translate(text)

def update_token_count(tokens: int):
    st.session_state.total_tokens = st.session_state.get('total_tokens', 0) + tokens
    st.session_state.total_cost = st.session_state.get('total_cost', 0) + tokens * 0.0001
...
async def load_chat_history_from_db(chat_name: str):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT role, content FROM chat_history WHERE chat_name = ? ORDER BY id", (chat_name,)) as cursor:
            st.session_state.messages = [{"role": row[0], "content": row[1]} async for row in cursor]
...
async def async_stream_openai_response(client, params: Dict[str, Any], messages: List[Dict[str, str]]):
    try:
...
async def async_stream_llm_response(client: Any, params: Dict[str, Any], messages: List[Dict[str, str]], provider: str) -> str:
    try:
        if provider == "Groq":
...
async def process_chat_input(prompt: str, client: Any) -> None:
    try:
        ...
        if st.session_state.file_content and not st.session_state.is_file_response_handled:
        ...
        if st.session_state.content_creation_mode:
            generated_content = await create_content(prompt, st.session_state.content_type)
            with st.chat_message("assistant"):
                st.markdown(f"## Generated {st.session_state.content_type}:\n\n{generated_content}")

        if st.session_state.show_summarization:
            summary = await summarize_text(st.session_state.file_content or prompt, st.session_state.summarization_type)
            with st.chat_message("assistant"):
...
def setup_sidebar() -> None:
    with st.sidebar:
        ...
        selected_provider = st.selectbox("Provider", available_providers, label_visibility="collapsed")
        ...
        with st.expander("Chat Settings", expanded=True):
            ...
            with col1:
                chat_name_input = st.text_input("Enter a name for this chat:", max_chars=50, label_visibility="collapsed", placeholder="Chat Name")
            with col2:
        ...
        with st.expander("Persona"):
            ...
            if st.session_state.persona == "Custom":
                st.session_state.custom_persona = st.text_area("Enter Custom Persona Description:", value=st.session_state.get("custom_persona", ""), height=100)
            else:
        ...
        with st.expander("Audio & Language"):
            ...
            st.session_state.voice = st.selectbox("Select Voice:", VOICE_OPTIONS["OpenAI"] if st.session_state.provider == "OpenAI" else VOICE_OPTIONS["gTTS"])
        ...
        with st.expander("Summarize"):
            ...
            if st.session_state.show_summarization:
                st.session_state.summarization_type = st.selectbox("Summarization Type:", ["Main Takeaways", "Main points bulleted", "Concise Summary", "Executive Summary"])
...
def handle_file_upload() -> None:
    uploaded_file = st.file_uploader("Upload a file", type=["pdf", "docx", "txt", "md", "jpg", "jpeg", "png", "xlsx", "pptx"])
    if uploaded_file:
        try:
...
async def main() -> None:
    ...
    if not is_connected():
        st.warning("You are currently offline. Loading previous chat history from local storage.")
