...
# Async Anthropic API Response Stream
# async def async_stream_anthropic_response(
...
#                         "model": params["model"],
#                         "max_tokens_to_sample": params.get("max_tokens"),
...
#             else:
#                 yield f"Error in Anthropic API call after retries: {str(e)}"
        ...
        try:
            ...
            async for chunk in response:
                if "completion" in chunk:
                    yield chunk["completion"]
            return
        except (anthropic.APIError, anthropic.ClientError) as e:
            logger.error(f"Error in Anthropic API call: {str(e)}")
            if attempt < retry_attempts - 1:
                await asyncio.sleep(2)
            else:
...
# Main LLM Response Stream - Now Includes Anthropic
async def async_stream_llm_response(
    ...
    try:
        if provider == "Groq":
            ...
            async for chunk in async_stream_anthropic_response(
                client, params, messages
            ):
                yield chunk
        else:
...
# Update API client function to include Anthropic
